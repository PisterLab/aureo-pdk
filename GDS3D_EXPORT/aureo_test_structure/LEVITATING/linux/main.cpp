...
int Wm_X11::main(int argc, char *argv[])
{
	...
	// };
   	...
   	vi_attr[i++] = GLX_RGBA;
   	vi_attr[i++] = GLX_DOUBLEBUFFER;
   	if (stereo)
      	vi_attr[i++] = GLX_STEREO;

   	/* Key/value attributes. */
   	vi_attr[i++] = GLX_RED_SIZE;
   	vi_attr[i++] = 1;
   	vi_attr[i++] = GLX_GREEN_SIZE;
   	vi_attr[i++] = 1;
   	vi_attr[i++] = GLX_BLUE_SIZE;
   	vi_attr[i++] = 1;
   	vi_attr[i++] = GLX_DEPTH_SIZE;
   	vi_attr[i++] = 1;
   	if (samples > 0) {
      		vi_attr[i++] = GLX_SAMPLE_BUFFERS;
      		vi_attr[i++] = 1;
      		vi_attr[i++] = GLX_SAMPLES;
      		vi_attr[i++] = samples;
   	}
