...
if ((-s $filename) && ($restart == 0)) {
    ...
    `$model_gen_src net-spec $exmpl.net $num_inputs  $num_hidden $num_outputs / - 0.05:0.5 0.05:0.5 - x0.05:0.5 - 100`;
    `$model_gen_src model-spec $exmpl.net real 0.05:0.5`;
    $trans_inputs = "/ ";
    ...
    `$model_gen_src  data-spec $exmpl.net $num_inputs $num_outputs / $exmpl.dat\@$tr_beg:$tr_end . $exmpl.dat\@$tst_beg:$tst_end . $trans_inputs $trans_targets`;
#    `$model_gen_src  data-spec $exmpl.net $num_inputs $num_outputs / $exmpl.dat\@$tr_beg:$tr_end . $exmpl.dat\@$tst_beg:$tst_end . $trans_inputs`;
    `$model_gen_src  net-gen $exmpl.net fix 0.5`;
    `$model_gen_src  mc-spec $exmpl.net repeat 10 sample-noise heatbath hybrid 500:20 0.05`;
    `$model_gen_src  net-mc $exmpl.net 1`;
}
$tmpfile = "$exmpl".".tmp";
$stepsize = 0.35;
for ($i = 2; $i <= $batch0; ++$i) {
  `$model_gen_src  mc-spec $exmpl.net repeat 10 sample-sigmas heatbath hybrid 200:25 $stepsize`;
  `$model_gen_src  net-mc $exmpl.net $i`;

  `$model_gen_src  net-plt t r $exmpl.net > $tmpfile`;
...
`$model_gen_src  mc-spec $exmpl.net repeat 10 sample-sigmas heatbath hybrid 200:20 $stepsize`;
`$model_gen_src  net-mc $exmpl.net $batch1`;
if ($stepsize > .15) {
    $stepsize *= .4;
}

$convergence_achieved = 0;
`$model_gen_src  mc-spec $exmpl.net repeat 10 sample-sigmas heatbath hybrid 200:10 $stepsize`;
for ($i = $batch1 + 1; $i <= $batch2; ++$i) {
  `$model_gen_src  net-plt t E $exmpl.net > $tmpfile`;
...
if ($convergence_achieved == 0) {
  `$model_gen_src  mc-spec $exmpl.net sample-sigmas heatbath hybrid 200:20 0.25`;
  `$model_gen_src  net-mc $exmpl.net $iter`;
}
