...
if ($stage <= 3) {
    ...
    `perl $script_dir/model_gen_adaptive.pl $model $num_inputs $num_hidden $num_outputs $tr_beg $tr_end $tst_beg $tst_end $iter 1 $model_gen_src`;
...
# Unsigned histogram
`perl $script_dir/preditn.pl $model $num_inputs $num_outputs $iter_start $iteration $model_gen_src`;
