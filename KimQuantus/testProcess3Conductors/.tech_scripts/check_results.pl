...
@limits = (0., 2.5, 5., 7.5, 10., 15., 20., 25.);

open(errfile, ">$outfile");

for ($i = 2; $i <= $#limits; $i++) {
    printf errfile "<%4.1f%   ", $limits[$i];
}
printf errfile ">%4.1f% \n", $limits[$#limits];
for ($i = 2; $i <= $#limits; $i++) {
    printf errfile "------   ";
...
#
	...
	for ($i = 2; $i <= $#limits; $i++) {
		...
		$histogram[$j][$i - 1] = $histogram[$j][$i - 1] + 1;
	...
	if ($flag == 0) {
	    $histogram[$j][$#limits] = $histogram[$j][$#limits] + 1;
	}
    }
}
close(infile);
...
for ($j = 1; $j <= $num_targets; $j++) {
    ...
    $variance = 0.;
    for ($i = 2; $i <= $#limits; $i++) {
	printf errfile "%-6d   ", $histogram[$j][$i - 1];
    }
...
sub compute_histogram {
    my ($dimension, @values, @limits, @histogram, $flag, $i, $j);
	...
	if ($flag == 0) {
	    $histogram[$#limits] = $histogram[$#limits] + 1;
	}
    }

    for ($i = 2; $i <= $#limits; $i++) {
	printf "%-6d   ", $histogram[$i - 1];
    }
